...
# cf. docs/ACIR-CANONICAL-COMPLETENESS.md. The validator no longer copies
...
class ACIRValidator:
    ...
    def _level1_jsonschema_v03(self, doc: dict):
        ...
        # to the LLM — a single /api/generate call can produce multiple
        # corrections if we report several errors at once.
            ...
            for call in _find_io_calls(dr["$inline"].get("module", {})):
                ...
                if tgt not in decl_deps:
                    if tgt not in name_set:
                        ...
                        # project component provides. No auto-stub —
    ...
    def _validate_operation(self, op: dict, path: str):
        ...
        if primitive == "F_SEQUENCE":
            ...
            if _is_v03(self._doc_version):
                # v0.3 canonical form ($defs/FBranch) : {condition, then,
                # else?} — NO `conditions` array. The JSON schema (level
    ...
    def _level2_v03_extensions(self, doc: dict):
        ...
        # _v03_check_ref_resolution rebuilds `valid_refs` from all
    ...
    def _v03_check_wildcard_scope(self, module: dict):
        ...
        def walk(node: Any, path: str = "$", in_allowed_scope: bool = False):
            if isinstance(node, dict):
                ...
                # at level 3. The "calculate" scope applies to the `inputs` of the calculate
                # not to the formula.
    ...
    def _v03_check_calculate_refs(self, module: dict):
        ...
        for unit in module.get("units", []) or []:
            ...
            for calc in calcs:
                ...
                # on level-2 error → CALCULATE_DSL_PARSE_ERROR would never
                # fire). Refs are only resolved on a well-formed formula.
                try:
